...
8. Ensure that you have included unit and integration tests for any new features added / bugs fixed.
9. Run `npm test`. If any tests fail, resolve the issue with the code and retry.
...
tests, and integration tests. At present, the following sub-tests can be run on a standalone basis:
...
* `npm run test-integration`: Checks Newman sanity with sample requests categorized by collection
...
`test/system`           | Formerly `infra` tests, checks for proper code structuring and division across the project
`test/integration`      | Contains the integration test runner and validates Newman as a library

### Branching and Tagging Policy

This repository uses a standard `git-flow` branch management policy/strategy. If you want to learn more about `git-flow`,
refer to this [tutorial from Atlassian](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow).
More details can be found at [http://nvie.com/posts/a-successful-git-branching-model/](http://nvie.com/posts/a-successful-git-branching-model).

> Deletion of `main` and `develop` and rebasing on `main` is blocked.
...
The following best practices, coupled with a pinch of common sense will keep the repository clean and usable in the future.
...
sanity and smoke tests. If any test fails, do not change the test to pass your commit. The tests are there with a
purpose. Discuss within your team to ensure that the changes that you do to test specs are valid. If you are adding a
new feature, accompanying them with new tests is a good practice.
...
test one method at a time. With regard to Newman, each integration test consists of a Postman collection and an optional
set of environment, data, and global files.

All these files are arranged without any directory segregation, as can be seen in [test/integration](https://github.com/postmanlabs/newman/tree/main/test/integration).
...
Commit to the `main` branch and `develop` branch is locked. As such, `git-flow` for feature completion and release will not
work. Thus, the last steps of feature completion in `git-flow` will happen as a Pull Request. Avoid changing the `version`
field in `package.json` for feature pull requests, as `version` bumps are handled separately.

1. Pull Request comments and commit comments should explicitly discuss what changes were made. The Pull Request reviewer
   should not need to communicate out of scope issues or have to clarify the pull request description in order to understand
...
3. Ensure that your feature branch has been tested and is associated with issues from the corresponding issue tracker.
   The issue must be in a "resolved" state, implying that the issue has been fully tested and accepted for inclusion.

4. Pull Requests with merge conflicts are very difficult to review and are at a higher likelihood of being rejected.
   Ensure that the `head` of your feature branch is either already merged with `develop` or has no conflict when it is
   merged with `develop`.

5. The turnaround time to close a Pull Request is directly proportional to the delta of changes done - more changes
   in files, more time it takes to review. As such, if you anticipate a feature branch having a large delta on feature
   completion, break it into sub-issues in the issue tracker, test them, close them, and then send a PR for that branch.

6. In addition, the turnaround time for a Pull Request is affected by unclear commit messages.

7. If you have deadlines to ensure feature completion, sendthe  Pull Request ahead of time. Better still, ensure that your
   feature development time window accounts for the PR acceptance time as well.

8. If you have mentioned issue tracker references in the Pull Request description, the severity and priority of those issues
   will be taken into account. Otherwise, no Pull Request will take priority over others already in the queue - it is
...
that can run on every local machine. In general, knowledge about these scripts is not necessary for development.
...
Please **refrain** from publicly posting about the security vulnerability in **any** form until its corresponding fix
has been released.

---
*Sections of this document use excerpts from various books and the Internet, among which, [this](http://git-scm.com/book)
