...
module.exports = {
     ...
     * @param {?Object} cache A set of item id - full name key value pairs.
     ...
     * @private
     */
    getFullName: function (item, options, cache, name, id) {
        var parent,
            cachedValue = cache && cache[id];
        ...
        if (item.id !== options.collection.id && cachedValue) { return cachedValue; }

        parent = this.parentOf(item); // @todo: Use item.parentOf when available

        return parent ?
            this.getFullName(parent, options, cache, (item.name || item.id) + (name ? SEP + name : E), id ||
                (!name && item.id)) : (cache[id] = name);
    },
