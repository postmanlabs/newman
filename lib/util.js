        ...
        return (/^https?:\/\/.*/).test(location) ?
            ...
            request.get({
                ...
                if ((response.statusCode !== 200) || (body && body.error)) {
                    ...
                    resource = _(url.parse(location).path).split('/').get(1).slice(0, -1) || 'resource';

                    return callback(_.assign(error, {
                        name: _.get(body, 'error.name'),
                        help: `Error fetching the ${resource} from the Postman Pro API. Ensure that the url is valid.`
                    }));
                }
