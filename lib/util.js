...
module.exports = {
     ...
     * @private
     */
    getFullName: function (item, options, name, id) {
        var parent = this.parentOf(item); // @todo: Use item.parentOf when available

        return parent ?
            this.getFullName(parent, options, (item.name || item.id) + (name ? SEP + name : E), id ||
                (!name && item.id)) : name;
    },
