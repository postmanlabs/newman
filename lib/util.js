...
util = {
    ...
    /**
     * Fetch the api key for the currently authenticated user from the OS keychain
     *
     * @param {Function} cb - The callback invoked when the authentication check has completed.
     * @todo: Update the method signature to include an optional account when supported, similar to login
     */
    getApiKey: function (cb) {
        keytar
            .getPassword(SERVICE, process.env.USER || 'user') // eslint-disable-line no-process-env
            .then(cb, cb)
            .catch(cb);
    },
