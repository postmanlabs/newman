...
RunSummary = function RunSummary (emitter, options = {}) {
    ...
    var summary = this,
        ...
        skipExecutionSummary = !_.get(options, 'executionsSummary', true) ||

            // in case of Newman being used as on CLI (via commander),
            // if only CLI reporter is specified, the summary is not needed
            // because everything is printed on the stdout. we can avoid
            // accumulating the summary.
            (
                options._isCli &&
                Array.isArray(options.reporters) &&
                options.reporters.length === 1 &&
                options.reporters[0] === 'cli'
            );
    ...
    // don't need trackers if not keeping execution summary
