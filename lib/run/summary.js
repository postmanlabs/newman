...
RunSummary = function RunSummary (emitter, options) {
    ...
    var summary = this,
        onlyCLIReporterEnabled,
        skipExecutionsSummary;

    // Need to do add this check for unit-tests when no options are passed
    if (options) {
        if (!options.skipExecutionsSummary) {
            onlyCLIReporterEnabled = (Array.isArray(options.reporters) &&
                options.reporters.length === 1 &&
                options.reporters.includes('cli'));
        }
        // skipExecutionsSummary in case the option is specified, or only cli is specified as reporter
        skipExecutionsSummary = options.skipExecutionsSummary || onlyCLIReporterEnabled;
    }
    ...
    // not attach reporting trackers in executions, if skipExecutionsSummary flag is enabled
    if (!skipExecutionsSummary) {
