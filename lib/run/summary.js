...
RunSummary = function RunSummary (emitter, options) {
    ...
    var summary = this,
        executionSummaryOptions = _.get(options, 'executionsSummary', true),
        skipExecutionSummary;

    if (!executionSummaryOptions) {
        // set skipExecutionSummary to true in case `--no-execution-summary` option is specified
        skipExecutionSummary = true;
    }
    else {
        // executionsSummary also in the case when only cli is specified as reporter
        skipExecutionSummary = (options && Array.isArray(options.reporters) &&
    ...
    // not attach reporting trackers in executions, if executionsSummary flag is enabled
    if (!skipExecutionSummary) {
