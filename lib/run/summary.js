...
RunSummary = function RunSummary (emitter, options) {
    ...
    if (options) {
        onlyCLIReporterEnabled = (_.isArray(options.reporters) &&
            options.reporters.length === 1 &&
            options.reporters.includes('cli'));
        // skipExecutions in case the option is specified, or only cli is specified as reporter
        skipExecutions = _.get(options, 'skipExecutions') || onlyCLIReporterEnabled;
    }
    ...
    // not attach reporting trackers in executions, if skipExecutions flag is enabled
    if (!skipExecutions) {
