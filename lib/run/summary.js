...
_.assign(RunSummary, {
    ...
    attachRequestTracker (summary, emitter) {
        ...
        emitter.on('request', function (err, o) {
            ...
            // compute average time of all phases of request
            timingPhases && _.forEach({
                waitAverage: 'wait',
                dnsAverage: 'dns',
                tcpAverage: 'tcp',
                firstByteAverage: 'firstByte',
                downloadAverage: 'download',
                totalAverage: 'total'
            }, function (value, key) {
                summary.run.timings[key] =
                    (summary.run.timings[key] * (requestCount - 1) + timingPhases[value]) / requestCount;
            });
        });
    },
