    ...
    attachRequestTracker (summary, emitter) {
        ...
        emitter.on('request', function (err, o) {
            ...
            timingPhases && _.forEach([
                ...
                var previousAverage = summary.run.timings[`${value}Average`],
                    previousVariance = Math.pow(summary.run.timings[`${value}Sd`], 2),
                    delta1 = timingPhases[value] - previousAverage,
                    delta2,
                    currentVariance;

                // compute average time for all phases of request
                summary.run.timings[`${value}Average`] =
                    (previousAverage * (requestCount - 1) + timingPhases[value]) / requestCount;

                // compute minimum time for all phases of request
                summary.run.timings[`${value}Min`] =
                    !summary.run.timings[`${value}Min`] ? timingPhases[value] : Math.min(summary.run.timings[`${value}Min`], timingPhases[value]);

                // compute maximum time for all phases of request
                summary.run.timings[`${value}Max`] = Math.max(summary.run.timings[`${value}Max`], timingPhases[value]);

                // compute standard deviation for all phases of request
                // refer Welford's online algorithm from https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance
                delta2 = timingPhases[value] - summary.run.timings[`${value}Average`];
