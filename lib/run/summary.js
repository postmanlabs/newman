    ...
    _.assign(summary, /** @lends RunSummary.prototype */ {
        ...
        run: {
            ...
            timings: {
                ...
                /**
                 * The average wait time for request of the run
                 *
                 * @type {number}
                 */
                waitAverage: 0,
                ...
                /**
                 * The average TCP connection time of the run
                 *
                 * @type {number}
                 */
                tcpAverage: 0,
                ...
                /**
                 * The average download time of the run
                 *
                 * @type {number}
                 */
                downloadAverage: 0,
    ...
    attachRequestTracker (summary, emitter) {
        ...
        emitter.on('request', function (err, o) {
            ...
            var size = _.isFunction(o.response.size) && o.response.size(),
                ...
                timingPhases = _.isFunction(o.response.timingPhases) && o.response.timingPhases(),
            ...
            summary.run.timings.waitAverage =
            (summary.run.timings.waitAverage * (requestCount - 1) + timingPhases.wait) / requestCount;

            // compute average DNS lookup time
            summary.run.timings.dnsAverage =
                (summary.run.timings.dnsAverage * (requestCount - 1) + timingPhases.dns) / requestCount;

            // compute average TCP connection time
            summary.run.timings.tcpAverage =
                (summary.run.timings.tcpAverage * (requestCount - 1) + timingPhases.tcp) / requestCount;

            // compute average first byte time
            summary.run.timings.firstByteAverage =
                (summary.run.timings.firstByteAverage * (requestCount - 1) + timingPhases.firstByte) / requestCount;

            // compute average download time
            summary.run.timings.downloadAverage =
                (summary.run.timings.downloadAverage * (requestCount - 1) + timingPhases.download) / requestCount;

            // compute average response time
            summary.run.timings.totalAverage =
                (summary.run.timings.totalAverage * (requestCount - 1) + timingPhases.total) / requestCount;
        });
    },
