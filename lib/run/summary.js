...
RunSummary = function RunSummary (emitter, options) {
    ...
    _.assign(summary, /** @lends RunSummary.prototype */ {
        ...
        run: {
            ...
            /**
             * Stores detailed information about the timings of each request in each iteration.
             *
             * @type {Array<Object>}
             */
            iterationData: []
    ...
    // Initialising the iterationCount if passed
    if (options && options.iterationCount) {
        iterationCount = options.iterationCount;
    }
    // If not passed explicitly
    else { iterationCount = 1; }
    // track run timings (start and end)
    RunSummary.attachTimingTrackers(this, emitter, iterationCount);
...
_.assign(RunSummary, {
    ...
    attachTimingTrackers (summary, emitter, iterationCount) {
        // mark the point when the run started
        // also mark the point when run completed and also store error if needed
        // initialise the iterationData array with the no. of objects determined using iterationCount
        emitter.on('start', function () {
            summary.run.timings.started = Date.now();
            // When iterationCount is passed or is 1.
            if (iterationCount >= 1) {
                summary.run.iterationData = new Array(iterationCount);
                for (var x = 0; x < iterationCount; x++) {
                    // Object initialised for each iteration in the array.
                    summary.run.iterationData[x] = { iteration: x + 1, requestTimings: [] };
                }
            }
            // Exception handling..
            else { summary.run.timings.iterationData = []; }
        });
    ...
    attachRequestTracker (summary, emitter) {
        ...
        emitter.on('request', function (err, o) {
            ...
            // Timing details added for each request in the current iteration.
            times = _.merge({ name: o.item.name }, timingPhases);
            summary.run.iterationData[o.cursor.iteration] &&
            summary.run.iterationData[o.cursor.iteration].requestTimings.push(times);
