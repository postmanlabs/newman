...
RunSummary = function RunSummary (emitter, options) {
    ...
    if (!executionSummaryOptions) {
        // set skipExecutionsSummary to true in case `--no-execution-summary` option is specified
        skipExecutionsSummary = true;
    }
    else {
        // skip executions summary also in the case when only cli is specified as reporter
        skipExecutionsSummary = (options && Array.isArray(options.reporters) &&
