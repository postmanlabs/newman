...
_.assign(RunSummary, {
    ...
    attachTimingTrackers (summary, emitter) {
        ...
        emitter.on('start', function () {
            ...
            summary.run.timings.iterations = [];
        });
        ...
        emitter.on('beforeIteration', function (_err, o) {
            summary.run.timings.iterations && summary.run.timings.iterations.push({
                iteration: o.cursor.iteration + 1, started: Date.now() });
        });
        emitter.on('iteration', function (_err, o) {
            if (summary.run.timings.iterations) {
                summary.run.timings.iterations[o.cursor.iteration].completed = Date.now();
            }
        });
    },
