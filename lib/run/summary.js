...
proxyPropertyList = function (options, name) {
    return {
         ...
         */
        toJSON: function () {
            var exportOption = _.get(options, name);

            return _.merge(_.pick(exportOption, ['id', 'name']), {
                values: _.map(_.get(exportOption, 'values', _.isArray(exportOption) ? exportOption : {}),
                function (value, key) {
                    return _.isObject(value) ? value : { key: key, name: key, value: value, type: 'string' };
                })
            });
        }
    };
