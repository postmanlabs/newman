var fs = require('fs'),
    ...
    /**
     * The list of valid custom reporters
     *
     * @type {Array}
     */
    validCustomReporters = ['teamcity'];
...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        runner.run(options.collection, {
            ...
            entrypoint: options.folder && { execute: options.folder },
            ...
            _.isArray(reporters) && _.forEach(reporters, function (reporterName) {
                ...
                if (!(_.isFunction(Reporter) && _.isObject(emitter.reporters[reporterName]))) {
                    ...
                    // print install instruction in case a valid custom reporter is missing
                    if (_.includes(validCustomReporters, reporterName)) {
                        console.warn(`  run 'npm install newman-reporter-${reporterName}' to install`);
                    }
                }
            });
