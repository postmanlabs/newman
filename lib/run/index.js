...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        // slice the parsed data based on the range specified from start to end - 1
        if (options.iterationDataRange) {
            let dataSize = options.iterationData.length,
                start = options.iterationDataRange[0],
                end = options.iterationDataRange[1];

            // check if start and end are in bounds
            start = util.isInt(start) && start >= 0 && start < dataSize ? start : 0;
            end = util.isInt(end) && end > 0 && end <= dataSize ? end : dataSize;
            options.iterationData = start < end ? options.iterationData.slice(start, end) : options.iterationData;
        }
