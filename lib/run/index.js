...
module.exports = function (options, callback) {
    ...
    // if collections list is provided then merging all the collection into a single collection
    if (_.isArray(options.collection)) {
        let obj = {}, // It represents that single collection
            i;
        ...
        for (i = 0; i < options.collection.length; i++) {
            obj.item.push({ name: options.collection.info.name, item: options.collection[i].item });
        }
        obj.info.name = 'combined';
        options.collection = obj; // changing optios.collection to contain only a single collection.
    }
