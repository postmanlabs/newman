...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        runner.run(options.collection, {
            ...
            environment: _(options.environment.values).keyBy('key').mapValues('value').value(),
            ...
            _.assignIn(callbacks, {
                ...
                // override the `done` event to fire the end callback
                    ...
                    var ENV_EXPORT_KEYS = ['id', 'name', 'synced', 'syncedFilename'];

                    _.forEach(['environment', 'globals'], function (item) {
                        // fetch the path name from options if one is provided
                        var path = _.get(options, _.camelCase(`export-${item}`)),
                            exportContent = emitter.summary[item].toJSON();
                        ...
                        path && emitter.exports.push({
                            ...
                            path: path,
                            content: item === 'environment' ? _.merge(exportContent, {
                                timestamp: Date.now(),
                                _postman_exported_at: (new Date()).toISOString(),
                                _postman_exported_using: util.userAgent
                            }, _.pick(_.get(options, item), ENV_EXPORT_KEYS)) : _.get(exportContent, 'values')
                        });
                    });
