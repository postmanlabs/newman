...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        if (options.iterationData && (options.iterationDataFrom || options.iterationDataTo)) {
            let dataSize = options.iterationData.length + 1,
                start = options.iterationDataFrom,
                end = options.iterationDataTo;

            start = start >= 1 && start < dataSize ? start : 1;
            end = end >= start && end <= dataSize ? end : dataSize;

            options.iterationData = options.iterationData.slice(start - 1, end);
        }
