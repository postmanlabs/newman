...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        if (_.has(options, 'proxyCredentials') || _.has(options, 'proxyUrl')) {
            if (!_.has(options, 'proxyUrl')) {
                return callback(new Error('newman: expecting proxy url in ip:port format'));
            }
            if (!_.has(options, 'proxyCredentials')) {
                return callback(new Error('newman: expecting proxy credentials in username:password format'));
            }
            // checking input format of url and credentials
            if (!_.get(options, 'proxyCredentials').match('[a-zA-z]*:[a-zA-z]*')) {
                return callback(new Error('newman: improper proxy credential format ;eg username:password'));
            }
            // eslint-disable-next-line no-useless-escape
            if (!_.get(options, 'proxyCredentials').match('\d*.\d*.\d*.\d*:\d+')) {
                return callback(new Error('newman: improper proxy url format ;eg ip:port'));
            }

            var proxyCredentials = _.get(options, 'proxyCredentials').split(':'),
                proxyUrl = _.get(options, 'proxyUrl').split(':');

            _.assign(options, {
                proxy: {
                    username: proxyCredentials[0],
                    password: proxyCredentials[1],
                    ip: proxyUrl[0],
                    // eslint-disable-next-line radix
                    port: parseInt(proxyUrl[1])
                }
            });
        }
        ...
        runner.run(options.collection, {
            ...
            proxies: _.has(options, 'proxy') ? new sdk.ProxyConfigList({}, {
                match: 'https+http://*/*',
