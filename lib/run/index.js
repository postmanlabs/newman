...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        if (_.has(options, 'proxy')) {
            // converting proxy credentials to base64 format
            var proxy = new Buffer.from(options.proxy),
                proxyB64 = proxy.toString('base64'),
                proxyCredentials = proxyB64,
                collectionRequests = options.collection.items.members;

            _.update(options, 'proxy', function () {
                return proxyB64;
            });

            // setting new proxy header into collection list
            collectionRequests.forEach(function (item) {
                var proxyHeaderKey = 'Proxy-Authorization',
                    proxyHeaderValue = 'basic ' + proxyCredentials,
                    proxyHeader = new header({ key: proxyHeaderKey, value: proxyHeaderValue }, 'proxyHeader');

                if (_.has(item.request, 'headers')) {
                    item.request.headers.members.push(proxyHeader);
                }
                else {
                    _.update(item.request, 'headers', function () {
