var _ = require('lodash'),
    ...
    /**
     * A helper method to validate a collection and return error in case of v1 format.
     *
     ...
     * @returns {*}
     */
    validateCollection = function (collection, callback) {
        var error = null;

        if (util.isV1Collection(collection)) {
            error = new Error('Newman >= v4 does not support the v1 collection format');
            error.friendly = 'Use the Postman Native app to export collections in the v2 format';
        }
    ...
    configLoaders = {
        ...
        collection: function (value, options, callback) {
            ...
            // if the collection has been specified as an object, returns error in case of v1 collection format
            if (_.isObject(value)) {
                return validateCollection(value, done);
            }
            ...
            externalLoader('collection', value, options, function (err, data) {
                ...
                return validateCollection(data, done);
            });
        },
        ...
        /**
         * Helper to cast string folder to an array.
         *
        ...
        folder: function (value, options, callback) {
            let folders = _.isString(value) ? [value] : value;
