var _ = require('lodash'),
    ...
    parseCsv = require('@postman/csv-parse'),
    util = require('../util'),
    ...
    configLoaders = {
        ...
        sslClientCertList: function (location, options, callback) {
            ...
            if (typeof location !== 'string') {
                return callback(new Error('newman: path for ssl client certificates list file must be a string'));
            }

            fs.readFile(location, function (err, value) {
                if (err) {
                    return callback(new Error(`newman: unable to read the ssl client certificates file "${location}"`));
                }
                ...
                try {
                    ...
                    return callback(new Error(`newman: the file at ${location} does not contain valid JSON data.`));
                }

                // ensure that `sslClientCertList` is an array
                if (!Array.isArray(value)) {
                    return callback(new Error('newman: expected ssl client certificates list to be an array.'));
                }
