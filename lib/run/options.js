var _ = require('lodash'),
    ...
    /**
     * The message displayed when a collection in the unsupported v1 format is provided.
     *
     * @const
     * @type {String}
     */
    V1_COLLECTION_ERROR_MESSAGE = 'Newman >= v7 does not support the v1 collection format',

    /**
     * The actionable hint displayed alongside the v1 collection format error.
     *
     * @const
     * @type {String}
     */
    V1_COLLECTION_ERROR_HINT = 'Use the Postman app to export collections in the v2 format',
    ...
    /**
     * A helper method to validate that a collection is in a format Newman supports, and return it.
     *
    ...
    processCollection = function (collection, callback) {
        if (util.isV1Collection(collection)) {
            const error = new Error(V1_COLLECTION_ERROR_MESSAGE);

            error.friendly = V1_COLLECTION_ERROR_HINT;
     ...
     * @param {Boolean} context.header - A boolean indicating if the field belongs to the header row.
     * @returns {String|Number|Date}
     */
    csvAutoParse = function (value, context) {
        if (context.header) {
            // column names are used verbatim, casting them would rename the variable
            return value;
        }
    ...
    configLoaders = {
        ...
        collection: function (value, options, callback) {
            ...
            // if the collection has been specified as an object, validate its format and return the result
            if (_.isObject(value)) {
                ...
                async.waterfall([
                    ...
                    (json, cb) => {
                        ...
                        parseCsv(data, {
                            ...
                            relax_quotes: true, // allow using quotes without escaping inside unquoted string
