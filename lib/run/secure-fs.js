 ...
 *
 * @param {*} fs - Node's fs module or module with similar interface
 ...
 */
function SecureFs (fs, workingDir, insecureFileRead = false, fileWhitelist = []) {
    this._fs = fs;
    this._path = path;
    ...
    // Attach all functions in fs to secure-fs except overridden functions
    Object.getOwnPropertyNames(fs).map((prop) => {
        if (typeof fs[prop] !== 'function' || SecureFs.prototype[prop]) {
            return;
        }
