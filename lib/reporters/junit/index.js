 ...
 * @param {Boolean=} reporterOptions.collectionAsClassName - Boolean flag to include the collection name as the class name of a test suite
 * @param {String=} reporterOptions.export - Optional custom path to create the XML report at.
 * @returns {*}
 */
JunitReporter = function (newman, reporterOptions, options) {
    ...
    newman.on('beforeDone', function () {
        ...
        var startedUnix = _.get(newman, 'summary.run.timings.started');
        var date = new Date(startedUnix);
        ...
        _.forEach(cache, function (executions, itemId) {
            ...
            if(reporterOptions.collectionAsClassName) {
                suite.att('classname', collection.name + ".postman_collection.json");
            }
