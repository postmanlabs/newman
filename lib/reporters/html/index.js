 ...
 * @returns {*}
 */
PostmanHTMLReporter = function (newman, options, runOpts) {
        ...
        var items = {},
            ...
            assertions = _.transform(executions, function (result, currentExecution) {
                ...
                if (!_.has(traversedRequests, executionId)) {
                    ...
                    items[reducedExecution.item.id].item.fullName = util.getFullName(reducedExecution.item, runOpts);
                }
            ...
            aggregator = function (item) {
                if (!_.isEmpty(item.items)) {
                    return {
                        folder: { id: item.id, name: util.getFullName(item, runOpts) },
                        contents: item.items.map(aggregator)
                    };
                }
