        ...
        var aggregations = [],
            ...
            assertions = _.transform(executions, function (result, currentExecution) {
                ...
                _.forEach(currentExecution.assertions, function (assertion) {
                    var assertionName = assertion.assertion,
                        defaultAggregation = { name: assertionName, passed: 0, failed: 0 };

                    // set the current aggregation object to a blank object if it doesn't exist
                    result[executionId][assertionName] = result[executionId][assertionName] || defaultAggregation;

                    ++result[executionId][assertionName][ASSERTION_STATE[_.get(assertion, 'error') === undefined]];
                });
            }, {});
