 ...
 * @param {Object} runOpts - The set of collection run options.
 * @param {Object} nameCache - A set of item id - full name key pairs.
 * @returns {*}
 */
PostmanHTMLReporter = function (newman, options, runOpts, nameCache) {
    ...
    newman.on('beforeDone', function () {
        var items = {},
            ...
            assertions = _.transform(executions, function (result, currentExecution) {
                ...
                if (!_.has(traversedRequests, executionId)) {
                    ...
                    items[reducedExecution.item.id].item.name = util.getFullName(reducedExecution.item, runOpts,
                        nameCache);
                }
            ...
            aggregator = function (item) {
                if (!_.isEmpty(item.items)) {
                    return { folder: { id: item.id, name: nameCache[item.id] }, contents: item.items.map(aggregator) };
                }
        ...
        // console.log(JSON.stringify(_.map(this.summary.collection.items.members, aggregator), null, 2));
        this.exports.push({
            name: 'html-reporter',
