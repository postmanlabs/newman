...
module.exports = function PostmanCLIReporter (emitter, options) {
    ...
    emitter.on('done', function () {
        ...
        if (size.width && (size.width > 20)) {
            ...
            colWidths[2] = parseInt((size.width - colWidths[0]) * 0.2, 10);
            colWidths[3] = parseInt(size.width - (colWidths[0] + colWidths[1] + colWidths[2] + 5), 10);
        }
        else {
            colWidths = undefined;
        }
        ...
        for (i = 0, ii = failures.length; i < ii; i++) {
            message = failures[i].error.message;
            failures[i].at && (message += '\n' + colors.gray('at ' + failures[i].at));

            failureTable.push([{
                hAlign: 'right',
                content: padLeft(Number(i + 1), failureFaceValue).toString()
            }, failures[i].source, failures[i].error.name, message]);
        }
