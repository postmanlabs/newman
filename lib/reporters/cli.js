...
module.exports = function PostmanCLIReporter (emitter, options) {
    var currentGroup = options.collection,
        failures = new Table(TABLE_FAILURE_CONFIG);
    ...
    emitter.on('prerequest', function (err, cur, results, item) {
        results && results.forEach(function (execution) {
    ...
    emitter.on('request', function (err, cur, _response, request, item, response) {
        // @todo - trap request errors
        print(colors.gray('[%d, %dms]') + '\n', response.code, response.responseTime);
    });

    emitter.on('test', function (err, cur, results, item) {
        results && results.forEach(function (execution) {
    ...
    emitter.on('done', function () {
        if (failures.length) {
            for (var i = 0, ii = failures.length; i < ii; i++) {
                failures[i].unshift({
                    hAlign: 'right',
                    content: padLeft(Number(i + 1), (failures.length.toString()).length).toString()
                });
            }
            print._indentPrefix = '';
            print('\n' + failures.toString() + '\n');
