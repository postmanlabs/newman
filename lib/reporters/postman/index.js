const _ = require('lodash'),
    ...
    uploadRun = require('./upload-run');

/**
 * Uploads the newman run data to Postman servers
 *
 * @param {Object} newman - The collection run object with event handling hooks to enable reporting.
 * @param {Object} _reporterOptions -  A set of postman cloud reporter specific run options.
...
function PostmanReporter (newman, _reporterOptions, collectionRunOptions) {
    newman.on('beforeDone', (error, o) => {
        ...
        if (!collection) {
            print.lf('Publishing run details to postman cloud is supported only for collections specified' +
                ' via postman public api link.');
