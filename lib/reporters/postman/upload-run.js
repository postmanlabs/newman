const _ = require('lodash'),
    ...
    { buildCollectionRunObject } = require('./helpers/run-utils');
 ...
 * 3. It also sends the run overview data in order to link it to an integration.
 *
 * @param {String} postmanApiKey - Postman API Key
...
function uploadRun (postmanApiKey, collectionRunOptions, runSummary, callback) {
    ...
    try {
        ...
        return callback(new Error(`Unable to serialize the run - ${error}`));
    }
