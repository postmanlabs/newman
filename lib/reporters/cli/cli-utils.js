...
cliUtils = {
    ...
    inspector: function (runOptions) {
        var width = Number(get(runOptions, 'reporters.cli.terminalWidth')),
            dimension = cliUtils.dimension(),
            options = {
                depth: 25,
                ...
                breakLength: width || ((dimension.exists && (dimension.width > 20)) ? dimension.width : width) - 16
            };
     ...
     *
     * @param {Object} cliReporterOptions - A set of options for the CLI reporter.
     * @returns {Function} - A sub-method to wrap content, given a piece of text, and indent value.
     */
    wrapper: function (cliReporterOptions) {
        var terminalWidth = Number(get(cliReporterOptions, 'terminalWidth')),
            dimension = cliUtils.dimension(),
            // note that similar dimension calculation is in utils.wrapper
            width = terminalWidth || ((dimension.exists && (dimension.width > 20)) ? dimension.width : 60) - 6;
