...
cliUtils = {
     ...
     * @param {Number} options.terminalWidth - The enforced terminal width as set in the run options.
     * @returns {Function} - A sub-method to wrap content, given a piece of text, and indent value.
     */
    wrapper: function (options) {
        var dimension = cliUtils.dimension(),
            // note that similar dimension calculation is in utils.wrapper
            width = options.terminalWidth || ((dimension.exists && (dimension.width > 20)) ? dimension.width : 60) - 6;
