 ...
 * @param {Boolean=} reporterOptions.showTimestamp - Boolean flag to log timestamp of each request, if set to true.
 * @param {Boolean=} reporterOptions.showTimestampNoTz - Boolean flag to log timestamp of each request, if set to true.
 * Here the timestamp is logged without printing the timezone offset of the user's system.
...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('beforeRequest', function (err, o) {
        ...
        if (reporterOptions.showTimestamp || reporterOptions.showTimestampNoTz) {
            timestamp = reporterOptions.showTimestampNoTz ?
                cliUtils.currentTime().substring(4, 24) : // Removing the day and all timezone info
                cliUtils.currentTime().substring(4, 33); // Only removing the timezone name (offset is logged)
            print('  %s  %s %s ',
