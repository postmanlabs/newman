...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('done', function () {
        ...
        if (!reporterOptions.noSummary) {
            print(LF + PostmanCLIReporter.parseStatistics(summary.run.stats, summary.run.timings,
                    summary.run.transfers) + LF);
        }

        // show the failures table (provided option does not say it is not to be shown)
        if (!reporterOptions.noFailures && summary.run.failures && summary.run.failures.length) {
            print(LF + PostmanCLIReporter.parseFailures(summary.run.failures).toString() + LF);
        }
    });
    ...
    emitter.on('assertion', function (err, o) {
        ...
        print.lf('%s %s', passed ? colors.green('  ✔ ') : colors.red.bold(pad(this.summary.run.failures.length, 3, SPC) +
