...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('request', function (err, o) {
        ...
        var size = o.response && o.response.size(),
            timingPhases = o.response && util.beautifyTime(o.response.timingPhases()),
            timingHeaders = {
                wait: 'Wait',
                dns: 'DNS Lookup',
                tcp: 'TCP Connection',
                firstByte: 'Start Transfer',
                download: 'Download',
                total: 'Total'
            };
        ...
        if (options.verbose && timingPhases) {
            timingTable = new Table({
                chars: cliUtils.cliTableTemplate_Compact,
                colWidths: [9],
                colAligns: _.fill(Array(_.size(timingPhases)), 'center')
            });

            // add name of phases in the table
            timingTable.push(Object.keys(timingPhases).map(function (phase) {
                return colors.info(timingHeaders[phase] || phase);
            }));

            //add time of phases in the table
            timingTable.push(Object.values(timingPhases).map(function (time) {
                return colors.log(time);
            }));
...
_.assignIn(PostmanCLIReporter, {
     ...
     * @param {String} timings.waitAverage - The average wait time for request of the run
     * @param {String} timings.dnsAverage - The average DNS lookup time of the run
     * @param {String} timings.tcpAverage - The average TCP connection time of the run
     * @param {String} timings.firstByteAverage - The average first byte time of the run
     * @param {String} timings.downloadAverage - The average download time of the run
     * @param {String} timings.totalAverage - The average response time of the run
    ...
    parseStatistics (stats, timings, transfers, options) {
        ...
        timings.waitAverage && summaryTable.push([{
            colSpan: 3,
            content: format('average wait time: %s', util.prettyms(timings.waitAverage)),
            hAlign: 'left'
        }]);

        timings.dnsAverage && summaryTable.push([{
            colSpan: 3,
            content: format('average DNS lookup time: %s', util.prettyms(timings.dnsAverage)),
            hAlign: 'left'
        }]);

        timings.tcpAverage && summaryTable.push([{
            colSpan: 3,
            content: format('average TCP connection time: %s', util.prettyms(timings.tcpAverage)),
            hAlign: 'left'
        }]);

        timings.firstByteAverage && summaryTable.push([{
            colSpan: 3,
            content: format('average first byte time: %s', util.prettyms(timings.firstByteAverage)),
            hAlign: 'left'
        }]);

        timings.downloadAverage && summaryTable.push([{
            colSpan: 3,
            content: format('average download time: %s', util.prettyms(timings.downloadAverage)),
            hAlign: 'left'
        }]);

        timings.totalAverage && summaryTable.push([{
            colSpan: 3,
            content: format('average response time: %s', util.prettyms(timings.totalAverage)),
            hAlign: 'left'
        }]);
