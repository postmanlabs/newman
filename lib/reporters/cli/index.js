...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('request', function (err, o) {
        ...
        reqSize = reqSize && (reqSize.header || 0) + (reqSize.body || 0) || 0;
        size = size && (size.header || 0) + (size.body || 0) || 0;

        if (options.verbose) {
            print.lf(SPC);
            print(`  ${o.response.code} ${reason} ${colors.gray(symbols.star)} `);
            print(`${util.prettyms(o.response.responseTime)} ${colors.gray('time')} ${colors.gray(symbols.star)} `);
            print(`${util.filesize(reqSize)}${colors.gray(symbols.up)} `);
            print(`${util.filesize(size)}${colors.gray(symbols.down)} ${colors.gray('size')} ${colors.gray(symbols.star)} `);
            print(`${o.request.headers.members.length}${colors.gray(symbols.up)} `);
            print(`${o.response.headers.members.length}${colors.gray(symbols.down)} ${colors.gray('headers')} ${colors.gray(symbols.star)} `);
            print(`${o.response.cookies.members.length} ${colors.gray('cookies')}`);
            print.lf(SPC + LF);
        } else {
