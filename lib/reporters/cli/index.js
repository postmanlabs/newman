...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('beforeItem', function (err, o) {
        ...
        if (itemGroup && (currentGroup !== itemGroup)) {
            folderChain = util.getFullName(itemGroup, options);
            ...
            !root && _.forEach(folderChain, function (folder) {
                directory = directory ? directory + SEP + folder : folder;
                print('\n%s %s', symbols.folder, colors.reset(directory));
            });
        ...
        o.item && print.lf('\n%s %s', (root ? symbols.root : symbols.sub), o.item.name);
    });
...
_.assignIn(PostmanCLIReporter, {
    ...
    parseFailures: function (failures, options) {
        ...
        _.forEach(failures, function (failure, index) {
            ...
            failure.source &&
                (message += format(colors.gray('\ninside "%s"'), util.getFullName(failure.source, options)));

            // augment message with item parent information
            failure.source && failure.parent &&
                (message += format(colors.gray(' of "%s"'), util.getFullName(failure.parent, options)));
