    ...
    emitter.on('request', function (err, o) {
        ...
        if (options.verbose && timingPhases) {
            timingTable = new Table({
                chars: _.assign({}, cliUtils.cliTableTemplate_Blank, { mid: '', middle: ' ' }),
                colWidths: [9],
...
_.assignIn(PostmanCLIReporter, {
    ...
    parseStatistics (stats, timings, transfers, options) {
        ...
        timings && _.forEach({
            ...
            waitAverage: 'average wait time:',
            dnsAverage: 'average DNS lookup time:',
            tcpAverage: 'average TCP connection time:',
            firstByteAverage: 'average first byte time:',
            downloadAverage: 'average download time:'
        }, function (value, key) {
            timings[key] && summaryTable.push([{
                colSpan: 3,
                content: format(`${value} %s`, util.prettyms(timings[key])),
                hAlign: 'left'
            }]);
        });
