 ...
 * @param {Boolean=} reporterOptions.noVerboseOutputLimit - If true, remove truncation size limit of verbose CLI output.
...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('request', function (err, o) {
        ...
        if (isNaN(bodyClipSize)) {
            // eslint-disable-next-line max-len
            print(LF + 'Invalid value passed with --reporter-cli-verbose-output-limit flag, defaulting to 2KB. For more info, view https://www.npmjs.com/package/bytes' + LF);
        ...
        if (resTextLen) {
            ...
            if (!reporterOptions.noVerboseOutputLimit && resTextLen > bodyClipSize) {
