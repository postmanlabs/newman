...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('request', function (err, o) {
        ...
        var timingTable,
            timings,
            timingPhases,
            reqSize = o.request && o.request.size() || {},
            resSize = o.response && o.response.size() || {},
            reason = o.response && o.response.reason(),
            mime = o.response && o.response.contentInfo() || {},
            resText, // populated only in verbose
            timingHeaders = {
                prepare: 'prepare',
                wait: 'wait',
                dns: 'dns-lookup',
            ...
            SEP_SYMBOL = colors.gray(symbols.star);

        if (options.verbose) {
            print.lf(SPC);
            print(`  ${o.response.code} ${reason} ${SEP_SYMBOL} `);
            print(`${util.prettyms(o.response.responseTime)} ${colors.gray('time')} ${SEP_SYMBOL} `);
            print(`${util.filesize(reqSize.total)}${colors.gray(symbols.up)} `);
            print(`${util.filesize(resSize.total)}${colors.gray(symbols.down)} ${colors.gray('size')} ${SEP_SYMBOL} `);
            print(`${o.request.headers.members.length}${colors.gray(symbols.up)} `);
            print(`${o.response.headers.members.length}${colors.gray(symbols.down)} ${colors.gray('headers')} ${SEP_SYMBOL} `);
            print.lf(`${o.response.cookies.members.length} ${colors.gray('cookies')}`);
            print(`  ${mime.contentType} ${SEP_SYMBOL} `);
            print(`${mime.mimeType} ${SEP_SYMBOL} `);
            print(`${mime.mimeFormat} ${SEP_SYMBOL} `);
            print.lf(`${mime.charset}`);


            resText = o.response.text();
            ...
            if (resText) {
                resText = wrap(resText, `  ${colors.gray(symbols.console.middle)} `);
                print.buffer(colors.gray(`  ${symbols.console.top}\n`), colors.gray(`  ${symbols.console.bottom}`))
            ...
            print.lf(colors.gray('[%d %s, %s, %s]'), o.response.code, o.response.reason(),
                util.filesize(resSize.total), util.prettyms(o.response.responseTime));
        }
        ...
        if (options.verbose && timings) {
            ...
            }, { headers: [], values: [colors.log(' ')] });

            // decorate timing table tracking
            timingPhases.headers.unshift('⧗'); // @todo pass via symbols
