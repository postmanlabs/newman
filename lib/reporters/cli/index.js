...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('done', function () {
        ...
        if (options.singleRequest && run.executions && run.executions.length && run.executions[0].response && run.executions[0].response.text()) {
            print(run.executions[0].response.text());
        }
...
_.assignIn(PostmanCLIReporter, {
    ...
    parseSingleRequestStatistics (run) {
        ...
        // eslint-disable-next-line max-len
        transfers && summaryTable.push([colors.green('Data received: %s (approx)'), util.filesize(transfers.responseTotal)]);
        ...
        timings && _.forEach({
            ...
            // eslint-disable-next-line max-len
            timings[`${key}Average`] && summaryTable.push([colors.green(label), util.prettyms(timings[`${key}Average`])]);
        });
