...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('done', function () {
        ...
        if (options.verbose) {
            print(PostmanCLIReporter.verboseSession(run));
            print('\n\n');
        }
        ...
        if (options.singleRequest &&
            ...
            if (run.executions[0].response.contentInfo().mimeFormat === 'xml' || run.executions[0].response.contentInfo().mimeFormat === 'html' ) {
...
_.assignIn(PostmanCLIReporter, {
    ...
    parseSingleRequestStatistics (run) {
        const summaryTable = new Table({
            ...
            verboseSession = run.verboseSession;
        ...
        timingPhases && _.forEach({
            ...
            timingPhases &&
            summaryTable.push([colors.green(label), util.prettyms(timingPhases[`${key}`]) + 'ms']);
        });
