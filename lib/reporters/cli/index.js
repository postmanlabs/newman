var path = require('path'),
    ...
    cliUtils = require(path.join(__dirname, '..', '..', 'symbols')),
    print = require('./print'),
    ...
    parentOf = function (item) {
        return _.get(item, '__parent.__parent');
    },
...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    symbols = cliUtils.symbolMap(options.disableUnicode);
    ...
    emitter.on('beforeItem', function (err, o) {
        ...
        if (itemGroup && (currentGroup !== itemGroup)) {
            ...
            // again and again
            !root && print('\n%s %s', symbols.folder, colors.reset(itemGroup.name));
