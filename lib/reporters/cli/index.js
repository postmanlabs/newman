var _ = require('lodash'),
     ...
     *
     * @param {PostmanItem} item - A PostmanItem instance whose parent is to be determined.
     * @returns {PostmanItemGroup} - The parent of the specified object.
     */
...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    var currentGroup = options.collection,
         ...
         * @param {?String} [name=''] The intermediate name for the specified item
         * @param {?String} [id=''] Placeholder id
         * @returns {String} The full name of the provided item, including prepended parent item names
         * @private
         */
        getFullName = function (item, name, id) {
            // If the current state is not pointed at the collection root, and a cache hit exists, return it
            // Useful in general, and necessary for collection runs with a large number of iterations
            if (item.id !== options.collection.id && nameCache[id]) { return nameCache[id]; }

            var parent = parentOf(item);

            return parent ?
                getFullName(parent, (item.name || item.id) + (name ? SEP + name : E), id || (!name && item.id)) :
                (nameCache[id] = name);
        };
    ...
    emitter.on('start', function () {
        nameCache = {}; // maintain a fresh cache for every collection run
    ...
    emitter.on('beforeItem', function (err, o) {
        ...
        if (itemGroup && (currentGroup !== itemGroup)) {
            folderChain = getFullName(itemGroup);
            folderChain = folderChain ? folderChain.split(SEP) : [];

            // we only print if the request is not directly under collection. no point printing collection name
            // again and again
            !root && _.forEach(folderChain, function (folder) {
                print('\n%s %s', symbols.folder, colors.reset(folder));
            });
        ...
        o.item && print.lf('\n%s %s', (root ?
            symbols.root : symbols.sub), colors.reset(getFullName(o.item)));
    });
