...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    // disable colors based on `noColor|color` option provided when using as a library.
    // in case of CLI, this is triggered internally by `colors` module via `--no-color` flag.
    // @todo deprecate noColor option with `color: false`.
