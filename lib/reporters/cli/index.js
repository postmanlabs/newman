 ...
 * @param {Object} nameCache - A set of item id - full name key pairs.
 * @returns {*}
 */
PostmanCLIReporter = function (emitter, reporterOptions, options, nameCache) {
    ...
    emitter.on('done', function () {
        ...
        if (!reporterOptions.noFailures && run.failures && run.failures.length) {
            print(LF + PostmanCLIReporter.parseFailures(run.failures, options).toString() + LF);
        }
    });
    ...
    emitter.on('beforeItem', function (err, o) {
        var folderChain,
            itemGroup = util.parentOf(o.item), // @todo: Use o.item.parentOf when available
        ...
        if (itemGroup && (currentGroup !== itemGroup)) {
            folderChain = util.getFullName(itemGroup, options, nameCache);
        ...
        o.item && print.lf('\n%s %s', (root ?
            symbols.root : symbols.sub), colors.reset(util.getFullName(o.item, options, nameCache)));
    });
...
_.assignIn(PostmanCLIReporter, {
     ...
     */
    parseFailures: function (failures, options) {
        ...
        _.forEach(failures, function (failure, index) {
            ...
            failure.source &&
                (message += format(colors.gray('\ninside "%s"'), util.getFullName(failure.source, options, cache)));

            // augment message with item parent information
            failure.source && failure.parent &&
                (message += format(colors.gray(' of "%s"'), util.getFullName(failure.parent, options, cache)));
