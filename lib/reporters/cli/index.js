...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('done', function () {
        ...
        function formatXML (inputXML, indent) {
            indent = indent || '\t';

            // PART 1: Add \n where necessary
            var xmlString = inputXML.replace(/^\s+|\s+$/g, ''), // trim it (just in case)
                // PART 2: indent each line appropriately
                tabs = '', // store the current indentation
                start = 0, // starting line
                i,
                line;
            ...
            for (i = start; i < xmlArr.length; i++) { // for each line
                line = xmlArr[i].replace(/^\s+|\s+$/g, ''); // trim it (just in case)
        ...
        // show the body of the response for single request
        if (options.singleRequest &&
            run.executions &&
            run.executions.length &&
            run.executions[0].response &&
            run.executions[0].response.text()) {
            if (run.executions[0].response.headers.all()
                .find((element) => element.key === 'Content-Type') // eslint-disable
                .value.includes('application/xml')) {
                const input = run.executions[0].response.text(),

                    prettyxml = formatXML(input);

                print(prettyxml);
            }

            else if (run.executions[0].response.headers.all()
                .find((element) => element.key === 'Content-Type') // eslint-disable
                .value.includes('application/json')) {
                const a = JSON.parse(run.executions[0].response.text());
        ...
        if (!reporterOptions.noSummary) {
            if (options.singleRequest) {
                print(LF + PostmanCLIReporter.parseSingleRequestStatistics(run) + LF);
            }
            else {
...
_.assignIn(PostmanCLIReporter, {
    ...
    parseSingleRequestStatistics (run) {
        ...
        timings && _.forEach({
            ...
            timings[`${key}Average`] &&
            summaryTable.push([colors.green(label), util.prettyms(timings[`${key}Average`])]);
        });
