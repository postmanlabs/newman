    ...
    _run = () => {
        var runParser = new argparse.ArgumentParser({
            ...
            Integer = function (arg) {
                var numEquivalent = Number(arg);

                if (!_.isSafeInteger(numEquivalent) || numEquivalent < 1) {
                    return new Error('The value of iteration count must be a positive integer.');
                }

                return numEquivalent.valueOf();
            };
