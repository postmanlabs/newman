...
var _ = require('lodash'),
     ...
     * 
     * @param {Function} callback - The callback function passed to rawOptions.
     * 
     * @param {Object} rawArgs - The rawArgs supploed to rawOptions to be passed to _run
     */
    commands = function (callback, rawArgs) {
        try {
            _run(rawArgs);
        }
        catch (err) {
            return callback(_.set(err, 'help'));
        }
    },
    ...
    formatOptions = function (options) {
        var command = options.rawArgs[0] !== undefined ? options.rawArgs[0] : options._name,
            optionsObj;
        if (command && (command.includes('version') || command.includes('-v') || command.includes('-V'))) {
            command = command.replace('--', '');
            command = command.replace('-v', 'version');
            command = command.replace('-V', 'version');
        }
        optionsObj = { [command]: {} };
    ...
    rawOptions = function (procArgv, programName, callback) {
        var legacyMode = (procArgv.length && _.startsWith(procArgv[0], '-') &&
                !_.includes(['--help', '-h', '-v', '--version', '-V'], procArgv[0])),
            reporterArgs,
            rawArgs,
            result,
            checkForColor;
        ...
        try {
            commands(callback, rawArgs.argv);
            ...
            if (_.isEmpty(procArgv) || procArgv.includes('-h') || procArgv.includes('--help')) {
