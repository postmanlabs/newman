var _ = require('lodash'),
    ...
    execute = (program) => {
        ...
        parser.addArgument(['-j', '--no-summary'], {
            help: 'Prohibits any output from being logged',
            type: Boolean,
            defaultValue: false
        });
        ...
        parser.addArgument(['-S', '--noTestSymbols'], {
            help: 'Disables tick and cross symbols for the output, reverting to PASS|FAIL instead',
            type: Boolean,
            defaultValue: false,
            action: 'storeTrue'
        });
        ...
        parser.addArgument(['-l', '--tls'], {
            help: 'Specifies whether to use TLS v1 or not',
            type: Boolean,
            defaultValue: false,
            action: 'storeTrue'
        });
        ...
        parser.addArgument(['-N', '--encoding'], {
            help: 'Specifies and encoding for the response',
            defaultValue: false,
            choices: ['ascii', 'utf8', 'utf16le', 'ucs2', 'base64', 'binary', 'hex']
        });

        parser.addArgument(['-o', '--outputFile'], {
            help: 'Path to file for writing output to'
        });

        parser.addArgument(['-O', '--outputFileVerbose'], {
            help: 'Path to file for writing the full collection run details to'
        });

        parser.addArgument(['-t', '--testReportFile'], {
            help: 'Path to file for writing JUnit XML results to'
        });

        parser.addArgument(['-i', '--import'], {
            help: 'Import a Postman backup file, and save collections, environments, globals, and data'
        });

        parser.addArgument(['-p', '--pretty'], {
            help: 'Enabled pretty print while displaying collection run information'
        });

        parser.addArgument(['-W', '--whiteScreen'], {
            help: 'Inverts the color scheme for collection run output',
            type: Boolean,
            defaultValue: false,
            action: 'storeTrue'
        });
        ...
        parser.addArgument(['-L', '--recurseLimit'], {
            help: 'Limits recursive variable resolution for collection runs',
            type: Integer,
            defaultValue: 20
        });

        parser.addArgument(['-s', '--stopOnError'], {
            help: 'Stops the runner with a non-zero exit code when any test assertion fails',
            type: Boolean,
            defaultValue: false,
            action: 'storeTrue'
        });
        ...
        parser.addArgument(['-x', '--exitCode'], {
            help: 'Continues running tests despite test failures, but exit with code 1',
            type: Boolean,
            defaultValue: false,
            action: 'storeTrue'
        });
    ...
    rawOptions = function (procArgv, program, callback) {
        ...
        legacyMode && (result = (function (legacy) {
            ...
            run.reporterHtmlOutput = legacy.html;
